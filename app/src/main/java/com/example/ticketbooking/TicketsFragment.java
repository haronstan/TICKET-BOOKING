...
public class Buyticket extends AppCompatActivity {
    ...
    private String userId;  // User ID for the currently logged-in user
            ...
            if (task.isSuccessful()) {
                ...
                if (userDocument.exists()) {
                    ...
                    for (TextView seat : selectedSeats) {
                        ...
                        seatData.put("email", email);
                        seatData.put("firstName", firstName);
                        seatData.put("lastName", lastName);
                        seatData.put("phoneNumber", phoneNumber);

                        db.collection("seats").document(seatId).set(seatData).addOnCompleteListener(task1 -> {
